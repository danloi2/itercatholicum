...
const prayerModules = import.meta.glob('@features/prayers/data/orationes/*.json', { eager: true });
...
export default function PrayersPage({ language }: PrayersPageProps) {
  ...
  const pageTitleNode = useMemo(
    ...
    [viewMode, selectedPrayer, pageTitle, prayers, selectedPrayerId, language, liturgicalHex]
  );

  useEffect(() => {
    setHeaderProps({ pageTitle: pageTitleNode, centerChildren: true });
