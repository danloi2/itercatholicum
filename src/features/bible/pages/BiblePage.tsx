...
import BibleDisplay from '@features/bible/components/BibleDisplay';
...
export default function BibleReaderPage({ language }: BibleReaderPageProps) {
  ...
  const { hex: liturgicalHex } = useTodayLiturgicalColor(language);
  ...
  useEffect(() => {
    async function loadContent() {
      ...
      try {
        ...
        const modules = selectedVersion === 'vulgata' ? bibleModules.vulgata : bibleModules.torres;
        const key =
          selectedVersion === 'vulgata'
    ...
    loadContent();
  }, [selectedBook, selectedVersion, selectedChapter, verses.end, bibleModules]);
  ...
  const pageTitleNode = useMemo(
    () => (
      ...
      >
        <div
          className={cn('flex items-center gap-1 md:gap-2', selectedBookId ? 'justify-end' : '')}
        >
          <button
            onClick={() => {
              setSelectedBookId('');
                      ...
                      <div className="p-1 bg-[#fdfbf7] border-[#c49b9b] shadow-xl rounded-md min-w-[150px] animate-in fade-in zoom-in-95 duration-200">
                        {hierarchy.testaments.map((t) => (
                          <button
                            key={t}
                            onClick={() => {
                              const firstBook = BIBLE_BOOKS.find(
                                (b) => b.testament[language] === t
                              );
                              if (firstBook) {
  ...
  useEffect(() => {
    setHeaderProps({ pageTitle: pageTitleNode });
  }, [pageTitleNode, setHeaderProps]);
